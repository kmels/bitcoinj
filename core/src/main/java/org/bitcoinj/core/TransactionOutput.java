...
public class TransactionOutput extends ChildMessage {
    ...
    public Coin getMinNonDustValue() {
        return getMinNonDustValue(getParams().getReferenceDefaultMinTxFee().multiply(3));
    }
    ...
    */
    ...
    public boolean isWatched(TransactionBag transactionBag) {
        try {
            ...
            log.debug("isWatched(). Could not parse tx output script: {}", e.toString());
            return false;
        }
    }
    ...
    public boolean isMine(TransactionBag transactionBag) {
        try {
            ...
            // Just means we didn't understand the output of this transaction: ignore it.
            log.debug("isMine(). Could not parse tx {} output script: {}", parent != null ? parent.getHash() : "(no parent)", e.toString());
            return false;
        }
    }
