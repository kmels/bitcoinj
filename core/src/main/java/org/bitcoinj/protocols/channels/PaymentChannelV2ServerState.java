...
public class PaymentChannelV2ServerState extends PaymentChannelServerState {
	...
	if (tx.getVersion() >= Transaction.FORKID_VERSION)
	    signature = tx.calculateWitnessSignature(0, serverKey, createP2SHRedeemScript(), tx.getInput(0).getConnectedOutput().getValue(), hashType, anyoneCanPay);
	else
	    signature = tx.calculateSignature(0, serverKey, createP2SHRedeemScript(), hashType, anyoneCanPay);
