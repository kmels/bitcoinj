...
public class NewDifficultyAdjustmentAlgorithmRulesChecker extends AbstractPowRulesChecker {
    ...
    private void checkNextCashWorkRequired(StoredBlock storedPrev, Block nextBlock, BlockStore blockStore) {
        ...
        try {
            ...
            BigInteger nextTarget = AbstractBitcoinCashParams.ComputeTarget(first, last);
	    if (networkParameters.getUseForkId()) {
		AbstractBitcoinCashParams ps = (AbstractBitcoinCashParams)networkParameters;
		ps.verifyDifficulty(nextTarget, nextBlock);
	    } else {
		throw new IllegalArgumentException("NetworkParameters must be bitcoin cash");
	    }
